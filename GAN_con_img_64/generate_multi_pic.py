...
from aegan_model import _netG1, _netD1, _netD2,reshape,recover, AEGAN_ResnetGenerator, AEGAN_ResnetDecoder, \
    NLayerDiscriminator
from csnet import CSNet_Lite, _netG2, CSNet,_netRS
from refiner_model import _RefinerG_64, _RefinerD_64, UnetGenerator
...
netG1.load_state_dict(torch.load('GAN_nor_auto refine_STEP2&3_64_3tuple_subrate_0.25_blocksize_32/netG1_epoch_128_0.698620.pth'))

#自编码去噪器
RefinerG = _RefinerG_64(nc, ngf)
RefinerG.load_state_dict(torch.load('Refine_nor_auto refine_STEP2&3_64_3tuple_subrate_0.25_blocksize_32/RefinerG_epoch_128_172.261749.pth'))
...
for i in range(pic_num):
    ...
    vutils.save_image(img, '%s/multi_pic/fake_sample_128_%s_%s.png' % (opt.outf, output_num,int(i)), nrow=4, normalize=True)
