...
from csnet import CSNet_Lite, _netG2, _netRS
...
parser.add_argument('--sub_dir1', default='imgStep1_csnet_lite_normalize_subrate_0.75_contrast_64', help='the sub directory 1 of saving images')
...
pickle_save_path = 'CSNet_lite_normalize_patchsize_96_3tuple' + '_subrate_' + str(opt.sub_rate) + '_blocksize_' + str(BLOCK_SIZE)
...
#导入训练数据集
train_set = TrainDatasetFromFolder('./patch_data_96/train', crop_size=CROP_SIZE, blocksize=BLOCK_SIZE)
...
#512*512
data_dir_64 = 'celeba-64/celeba-64'          # this path depends on your computer
dset_64 = TestDatasetFromFolder(data_dir_64, blocksize=BLOCK_SIZE)
...
if __name__=="__main__":
    ...
    for epoch in range(LOAD_EPOCH, NUM_EPOCHS + 1):
        ...
        #图片重构对比
        test_img_real = dset_64[1][0].cuda()
        if epoch == 0:
            save_image(test_img_real, '%s/%s/real_samples.png' % (opt.outf, opt.sub_dir1), nrow=1)

        if epoch % 5 ==0:
            fake = netG2(netRS(test_img_real.view(1, 3, 64, 64)))
