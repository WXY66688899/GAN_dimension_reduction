...
net = CSNET_fc_3(IMAGE_SIZE*3, MEASURE_SIZE*3)
net.load_state_dict(torch.load('CSNET_fc3_celeba_64_subrate_0.75/net_epoch_50_0.228756.pth'))
#定义损失
...
save_img = 'celeba_64_fc3_contrast'
if not os.path.exists(save_img):
    os.makedirs(save_img)
save_image(test_img_real, save_img + '/real_samples.png' , nrow=1)

fake = un_norm(net(test_img_real.view(1,4096*3)).view(1,3,64,64))
...
for p in range(epoch_num):
    ...
    for i in seed:
        ...
        img = dset_test[i][0].reshape(inputsize, inputsize, 3)
        img = un_norm(img).numpy()
        # 进入CSNET查看恢复情况
        immg = immg.cuda()
        immg_recon = net(immg).reshape(inputsize, inputsize, 3)
