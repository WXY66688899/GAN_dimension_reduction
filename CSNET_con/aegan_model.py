...
class _netG1_fc(nn.Module):
    def __init__(self, g_input_dim, g_output_dim):
        super(_netG1_fc, self).__init__()
...
class _netD1_fc(nn.Module):
    def __init__(self, d_input_dim):
        super(_netD1_fc, self).__init__()
...
class _netG1(nn.Module):
    def __init__(self, nz, ngf, nm, batchsize, padding_type='reflect', norm_layer=nn.BatchNorm2d, use_dropout=False, use_bias=False):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(self.ngf * 2, self.nm, 4, 2, 1, bias=False),
            #nn.Tanh()
...
class _netD1(nn.Module):
    def __init__(self, ndf, nm):
        super(_netD1, self).__init__()
        self.ndf = ndf
        self.nm = nm
        self.main = nn.Sequential(
            # state size. (1) x 32 x 32
            nn.Conv2d(self.nm, self.ndf * 2, 4, 2, 1, bias=False),
...
z= D1(x)
print(z.size())
noise = torch.randn(10, 3,100)
G1 = _netG1_fc(100,28*28)
y=G1(noise)
print(y.size())'''