...
class _RefinerD_64(nn.Module):
    def __init__(self, nc, ndf):
        super(_RefinerD_64, self).__init__()
        self.nc = nc
        self.ndf = ndf
        self.main = nn.Sequential(
...
class _RefinerD_128(nn.Module):
    def __init__(self, nc, ndf):
        super(_RefinerD_128, self).__init__()
        self.nc = nc
        self.ndf = ndf
        self.main = nn.Sequential(
...
# For input size input_nc x 64 x 64
class _RefinerG_64(nn.Module):
    def __init__(self, nc, ngf):
        super(_RefinerG_64, self).__init__()
        ...
        self.conv2 = nn.Conv2d(ngf, ngf * 2, 4, 2, 1)
        self.conv3 = nn.Conv2d(ngf * 2, ngf * 4, 4, 2, 1)
        self.conv4 = nn.Conv2d(ngf * 4, ngf * 4, 4, 2, 1)
        self.conv5 = nn.Conv2d(ngf * 4, ngf * 4, 4, 2, 1)
        self.conv6 = nn.Conv2d(ngf * 4, ngf * 4, 4, 2, 1)

        self.dconv1 = nn.ConvTranspose2d(ngf * 4 , ngf * 4, 4, 2, 1)
        self.dconv2 = nn.ConvTranspose2d(ngf * 4 , ngf * 4, 4, 2, 1)
        self.dconv3 = nn.ConvTranspose2d(ngf * 4 , ngf * 4, 4, 2, 1)
        self.dconv4 = nn.ConvTranspose2d(ngf * 4 , ngf * 2, 4, 2, 1)
        self.dconv5 = nn.ConvTranspose2d(ngf * 2 , ngf, 4, 2, 1)
        self.dconv6 = nn.ConvTranspose2d(ngf, nc, 4, 2, 1)
    ...
    def forward(self, input):
        ...
        e4 = self.batch_norm4(self.conv4(self.leaky_relu(e3)))
        # state size is (ngf x 4) x 4 x 4
        e5 = self.batch_norm4(self.conv5(self.leaky_relu(e4)))
        # state size is (ngf x 4) x 2 x 2
        e6 = self.conv6(self.leaky_relu(e5))
        # state size is (ngf x 4) x 1 x 1

        # Decoder
        # Deconvolution layers:
        # state size is (ngf x 4) x 1 x 1
        d1 = self.dropout(self.batch_norm4(self.dconv1(self.relu(e6))))
        # state size is (ngf x 4) x 2 x 2
        d2 = self.dropout(self.batch_norm4(self.dconv2(self.relu(d1))))
        # state size is (ngf x 4) x 4 x 4
        d3 = self.dropout(self.batch_norm4(self.dconv3(self.relu(d2))))
        # state size is (ngf x 4) x 8 x 8
        d4 = self.batch_norm2(self.dconv4(self.relu(d3)))
        # state size is (ngf x 2) x 16 x 16
        d5 = self.batch_norm(self.dconv5(self.relu(d4)))
        # state size is (ngf) x 32 x 32
        d6 = self.dconv6(self.relu(d5))
        # state size is (nc) x 64 x 64
        output = self.tanh(d6)
        return output

# For input size input_nc x 64 x 64
class _RefinerG_128(nn.Module):
    def __init__(self, nc, ngf):
        super(_RefinerG_128, self).__init__()
        ...
        self.dconv2 = nn.ConvTranspose2d(ngf * 8 , ngf * 8, 4, 2, 1)
        self.dconv3 = nn.ConvTranspose2d(ngf * 8 , ngf * 8, 4, 2, 1)
        self.dconv4 = nn.ConvTranspose2d(ngf * 8 , ngf * 4, 4, 2, 1)
        self.dconv5 = nn.ConvTranspose2d(ngf * 4 , ngf*2, 4, 2, 1)
        self.dconv6 = nn.ConvTranspose2d(ngf*2, ngf, 4, 2, 1)
        self.dconv7 = nn.ConvTranspose2d(ngf, nc, 4, 2, 1)
    ...
    def forward(self, input):
        ...
        # Deconvolution layers:
        # state size is (ngf x 8) x 1 x 1
        d1 = self.dropout(self.batch_norm8(self.dconv1(self.relu(e7))))
        # state size is (ngf x 8) x 2 x 2
        d2 = self.dropout(self.batch_norm8(self.dconv2(self.relu(d1))))
        # state size is (ngf x 8) x 4 x 4
        d3 = self.dropout(self.batch_norm8(self.dconv3(self.relu(d2))))
        # state size is (ngf x 8) x 8 x 8
        d4 = self.batch_norm4(self.dconv4(self.relu(d3)))
        # state size is (ngf x 4) x 16 x 16
        d5 = self.batch_norm2(self.dconv5(self.relu(d4)))
        # state size is (ngf x 2) x 32 x 32
