...
class _netG1_64(nn.Module):
    def __init__(self, nz,ngf):
        super(_netG1_64, self).__init__()
        self.nz = nz
        self.ngf = ngf

        # Input is the latent vector Z.
        self.tconv1 = nn.ConvTranspose2d(self.nz, self.ngf*8, kernel_size=4, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(self.ngf*8)

        # Input Dimension: (ngf*8) x 4 x 4
        self.tconv2 = nn.ConvTranspose2d(self.ngf*8, self.ngf*4, 4, 2, 1, bias=False)
        self.bn2 = nn.BatchNorm2d(self.ngf*4)

        # Input Dimension: (ngf*4) x 8 x 8
        self.tconv3 = nn.ConvTranspose2d(self.ngf*4, self.ngf*2, 4, 2, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(self.ngf*2)

        # Input Dimension: (ngf*2) x 16 x 16
        self.tconv4 = nn.ConvTranspose2d(self.ngf*2, self.ngf, 4, 2, 1, bias=False)
        self.bn4 = nn.BatchNorm2d(self.ngf)

        # Input Dimension: (ngf) * 32 * 32
        self.tconv5 = nn.ConvTranspose2d(self.ngf, 3, 4, 2, 1, bias=False)
...
class _netG1_64_(nn.Module):
    def __init__(self, nz, ngf, batchsize, padding_type='reflect', norm_layer=nn.BatchNorm2d, use_dropout=False, use_bias=False):
        super(_netG1_64_, self).__init__()
        self.nz = nz
        self.ngf = ngf
        self.main = nn.Sequential(
            # input is Z, going into a convolution
...
class _netD1_64_(nn.Module):
    def __init__(self, ndf):
        super(_netD1_64_, self).__init__()
        self.ndf = ndf
        self.main = nn.Sequential(
            # state size. (1) x 64 x 64
            nn.Conv2d(3, self.ndf * 2, 4, 2, 1, bias=False),
...
class _netD1_64(nn.Module):
    def __init__(self,nc, ndf):
        super(_netD1_64, self).__init__()
        self.ndf = ndf
        self.nc = nc
