...
from aegan_model import _netG1, _netD1, _netD2,reshape,recover, AEGAN_ResnetGenerator, AEGAN_ResnetDecoder, \
    NLayerDiscriminator
from csnet import CSNet_Lite, _netG2, CSNet,_netRS, CSNet,_netG2_block
...
parser.add_argument('--outf', default='nobase_0.0001generate_pic', help='folder to output images and model checkpoints')
...
parser.add_argument('--sub_dir3', default='generate_144', help='the sub directory 3 of saving images')
...
netG1.load_state_dict(torch.load('img_128_3_tuple_csnet_subrate_0.0625+gan_con_32/nobase_model_gan_0.0001_refine_0.0001/nobase_GAN_32_nor_auto refine_STEP2&3_128_3tuple_lr_0.0001_subrate_0.0625_blocksize_32/netG1_epoch_156_0.966802.pth'))

#自编码去噪器
RefinerG = _RefinerG_128(nc, ngf)
RefinerG.load_state_dict(torch.load('img_128_3_tuple_csnet_subrate_0.0625+gan_con_32/nobase_model_gan_0.0001_refine_0.0001/nobase_Refine_nor_auto refine_STEP2&3_128_3tuple_0.0001_subrate_0.0625_blocksize_32/RefinerG_epoch_156_38.671631.pth'))
...
for i in range(pic_num):
    output_num = 16
    noise = torch.randn(output_num, nz, 1, 1).cuda()
    ...
    vutils.save_image(img, '%s/multi_pic/fake_sample_156_%s_%s.png' % (opt.outf, output_num,int(i)), nrow=4, normalize=True)
