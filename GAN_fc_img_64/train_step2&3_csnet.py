...
parser.add_argument('--sub_dir3', default='imgStep2&3_3_csnet_lite_subrate_0.191_gan_fc_nor', help='the sub directory 3 of saving images')
...
# 保存生成的图片
model_save_path2,model_save_path3 = os.path.join(opt.outf, opt.sub_dir2),os.path.join(opt.outf, opt.sub_dir3)
...
#存储gan模型
pickle_save_path = os.path.join(opt.outf,'GAN_fc_2&3_nor_64+csnet_lite' + '_subrate_' + str(opt.sub_rate) + '_blocksize_' + str(BLOCK_SIZE))
if not os.path.exists(pickle_save_path):
    os.makedirs(pickle_save_path)

#存储refine模型
refine_save_path = os.path.join(opt.outf, 'REFINE_2&3_nor_64+csnet_lite' + '_subrate_' + str(opt.sub_rate) + '_blocksize_' + str(BLOCK_SIZE))
...
#导入数据
data_dir_64 = 'E:/CSNET_con/celeba_64'
dataroot = data_dir_64
...
#判别32*32*64的discriminator
if opt.gan_type == 'GAN':
    netD1 = _netD1_fc(MEASURESIZE)
else:
...
noise = torch.FloatTensor(opt.batchSize, 3, nz)
fixed_noise = torch.FloatTensor(opt.batchSize, nz, 1, 1).uniform_(-1, 1)
label = torch.FloatTensor(opt.batchSize,3,1)
label_re = torch.FloatTensor(opt.batchSize)
...
optimizerRefinerG = optim.Adam(RefinerG.parameters(), lr=opt.lr*0.1, betas=(opt.beta1, 0.999))
optimizerRefinerD = optim.Adam(RefinerD.parameters(), lr=opt.lr*0.1, betas=(opt.beta1, 0.999))
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.niter_stage2):
        for i, data in enumerate(dataloader, 0):
            ...
            # design noise
            noise = torch.randn(batch_size, 3, nz).cuda()
            ################################################################
            ...
            output_RS = RESHAPE(output_RS).view(batch_size, 3, 28 * 28)
            ...
            print('Train G1: [%d/%d][%d/%d] Loss_D1: %.4f Loss_G1: %.4f D1(x): %.4f D1(G1(z)): %.4f'
                  % (epoch, opt.niter_stage2, i, len(dataloader),
                     errD1.item(), errG1.item(), D1_x, D1_G1_z1))
            # if i % 100 == 0 and opt.dataset!='lsun':
            if i % 100 == 0:
                # vutils.save_image(real_cpu,'%s/%s/real_samples.png' % (opt.outf, opt.sub_dir2), normalize=True)
                fake = netG2(RECOVER(netG1(noise).view(batch_size,3,28,28)))
                vutils.save_image(fake, '%s/%s/fake_samples_epoch_%s.png' % (opt.outf, opt.sub_dir2, epoch),
                                  normalize=True)

            # train with real
            RefinerD.zero_grad()
            fake_input = netG2(RECOVER(netG1(noise).view(batch_size,3,28,28)))
            # label.data.resize_(batch_size).fill_(real_label)
            label_re.resize_(1).fill_(real_label)
            output = RefinerD(input)
            # errRefinerD_real = criterion(output, label)
            # errRefinerD_real = criterion(output, label.expand_as(output))
            errRefinerD_real = GANLoss(output, label_re, criterion, criterion_l2, opt.gan_type)
            ...
            label_re.data.fill_(fake_label)
            ...
            errRefinerD_fake = GANLoss(output, label_re, criterion, criterion_l2, opt.gan_type)
            ...
            # errRS.backward()
            label_re.data.fill_(real_label)  # fake labels are real for generator cost
            ...
            errRefinerG = GANLoss(output, label_re, criterion, criterion_l2, opt.gan_type) + opt.lamb * criterion_l1(
            ...
            fake = RefinerG(netG2(RECOVER(netG1(noise).view(batch_size,3,28,28))))
            # if i % 100 == 0 and opt.dataset!='lsun':
            if i % 100 == 0:
        ...
        # for saving model
        if epoch % 2 == 0:
            torch.save(netD1.state_dict(), pickle_save_path + '/netD1_epoch_%d_%6f.pth' % (epoch, errD1.item()))
            torch.save(netG1.state_dict(), pickle_save_path + '/netG1_epoch_%d_%6f.pth' % (epoch, errG1.item()))
            ...
            torch.save(netD1.state_dict(), pickle_save_path + '/netD1_final.pth')
            torch.save(netG1.state_dict(), pickle_save_path + '/netG1_final.pth')
