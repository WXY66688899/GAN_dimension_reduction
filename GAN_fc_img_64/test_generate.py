...
parser.add_argument('--outf', default='3_tuple_no_tanh', help='folder to output images and model checkpoints')
...
parser.add_argument('--sub_dir3', default='test_img', help='the sub directory 3 of saving images')
...
netG1.load_state_dict(torch.load('3_tuple_no_tanh/GAN_fc_2&3_nor_64+csnet_lite_subrate_0.191_blocksize_32/netG1_epoch_52_1.465543.pth'))

#自编码去噪器
RefinerG = _RefinerG_64(nc, ngf)
RefinerG.load_state_dict(torch.load('3_tuple_no_tanh/REFINE_2&3_nor_64+csnet_lite_subrate_0.191_blocksize_32/RefinerG_epoch_52_44.229023.pth'))
...
noise = torch.randn(showsize, 3, nz).cuda()
img_unrefine = netG2(RECOVER(netG1(noise).view(showsize,3,28,28)))
img = RefinerG(netG2(RECOVER(netG1(noise).view(showsize,3,28,28))))
save_path = os.path.join(opt.outf, opt.sub_dir3)
if not os.path.exists(save_path):
    os.makedirs(save_path)
vutils.save_image(img_unrefine, '%s/%s/fake_samples_unrefine_epoch_52_2.png' % (opt.outf, opt.sub_dir3), nrow=4,normalize=True)
vutils.save_image(img, '%s/%s/fake_samples_epoch_52_2.png' % (opt.outf, opt.sub_dir3), nrow=4,normalize=True)